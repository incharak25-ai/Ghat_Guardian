...
export default function SOSScreen({ route, navigation }) {
  ...
  useEffect(() => {
    // Urgent vibration — cannot be dismissed accidentally
    Vibration.vibrate([500, 200, 500, 200, 1000], true);

    // Play loud alarm even in silent mode
    playSiren();
  ...
  async function playSiren() {
    try {
      // Override silent mode — critical for emergency
      ...
      // Use a bundled siren MP3 or a public domain alarm URL
      // For production, bundle a siren.mp3 in your assets folder
      const { sound: s } = await Audio.Sound.createAsync(
        { uri: 'https://www.soundjay.com/misc/sounds/fail-buzzer-02.mp3' },
        { shouldPlay: true, isLooping: true, volume: 1.0 }
      );
      setSound(s);
    } catch (e) {
      // If audio fails, vibration alone is the fallback
